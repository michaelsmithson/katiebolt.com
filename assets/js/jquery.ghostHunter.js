...
(function( $ ) {
	...
	$.fn.ghostHunter.defaults = {
		...
		rss 				: "/rss",
		onKeyUp 			: false,
		result_template 	: "<div class='search-title'><a href='{{link}}'><h2>{{title}}</h2><time datetime='{{pubDate}}'>{{pubDate}}</time></a></div>",
			...
			$.get(rssURL,function( data ){
		    	...
		    	for (var i = 0; posts && i < posts.length; i++) {
			        ...
			        var parsedData 	= {
						...
						pubDate 	: moment(post.find('pubDate').text()).format('DD MMMM YYYY'),
